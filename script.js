...
function processSegmentation(canvas, segmentation) {
  ...
  console.log(segmentation[0]);
  if (segmentation[1] !== undefined) {
    minX = segmentation[1].bbox[0];
    console.log(minX);
    
    minY = segmentation[1].bbox[1];
    console.log(minY);
    
    maxX = segmentation[1].bbox[0] + segmentation[1].bbox[2];
    
    maxY = segmentation[1].bbox[1] + segmentation[1].bbox[3];
  ...
  //     console.log(segmentation[0]);
  //     // segmentation has bbox,class(object name), score(match)
  //     // bbox is [x, y, width, height]
  //     console.log
  //     if (segmentation[0].bbox!== null) {
  //       if(x < minX) {
  //         minX = x;
  //         console.log(minX);
  //       }
...
function predictWebcam() {
  if (previousSegmentationComplete) {
    ...
    cocoSsd.load().then(model => {
      // 物体検出
      model.detect(videoRenderCanvas).then(predictions => {
        console.log('Predictions: ', predictions);
        processSegmentation(webcamCanvas, predictions);
        previousSegmentationComplete = true;
      });
    });
  }
